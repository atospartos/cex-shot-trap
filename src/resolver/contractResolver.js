...
class ContractResolver {
        ...
        try {
            ...
            if (response.data.symbols.length > 0) {
                ...
                if (symbolInfo.status === '1' || symbolInfo.status === 1) {
                    return {
                        currency: currency,
                        contractAddress: await this.extractContractAddress(currency, symbolInfo)
                    };
                } else {
    ...
    async extractContractAddress(symbol, symbolInfo) {
        ...
        const isLiquid = await dexClient.fetchTokenData(contractAddress)
        if (isLiquid.length === 0) {
            logger.info(`⚠️Нет ликвидности на DEX`);
        }

        // Проверяем контракт
        if (isLiquid.length > 0 && contractAddress && contractAddress !== '') {
    ...
    static formatTokenEntry(symbol, address) {
        if (!address) {
            return ``;
        }
    ...
    async updateFromFile(inputFile, outputPath) {
        ...
        const entries = uniqueTokens
            .map(t => ContractResolver.formatTokenEntry(t.symbol, t.address))
            .join(',\n\n');

        const fileContent = `// auto-generated by ContractResolver
// Generated: ${new Date().toISOString()}
// Total tokens: ${uniqueTokens.length}
